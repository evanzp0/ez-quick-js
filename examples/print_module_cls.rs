...
use anyhow::Error;
use ez_quick_js::ffi::{
    js_free_value, js_is_exception, js_new_int32, js_to_string,
    JSCFunctionEnum_JS_CFUNC_constructor, JSClassDef, JSClassID, JSModuleDef, JSRuntime,
    JS_GetOpaque, JS_GetOpaque2, JS_GetPropertyStr, JS_GetRuntime,
    JS_NewCFunction2, JS_NewClass, JS_NewObject, JS_NewObjectProtoClass,
...
unsafe extern "C" fn init_module_inner(ctx: *mut JSContext, m: *mut JSModuleDef) -> c_int {
    ...
    /*
        // 上面的操作，看上去很麻烦，其实写成 js 的 prototype 继承就很好理解了
        const proto = {
            test_func() {
                this.x * this.y
            },
        ...
        // 这里用 Object.assign() 把 proto 所有的属性值赋值到了 Print.prototype 属性上
        // Object.assign() 等价于: Print.prototype = proto; proto.constructor = Print
        Object.assign(Print.prototype, proto);

        // 以后就可以在 Print 生成的对象上调用 test_func 方法了
        let print_obj = new Print(2);
        print_obj.test_func();
    */
...
fn main() -> Result<(), Error> {
    // load js script
    let file_name = "./examples/print_module_cls.js";
